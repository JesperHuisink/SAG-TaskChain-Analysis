	...
	namespace Global {
		...
		public:
			...
			Schedule_state(const unsigned int num_processors, const State_space_data<Time>& state_space_data)
				...
				, tc_data{ init_task_chain_data_structure(state_space_data.get_task_chains(),state_space_data.get_num_cpus()>1) }
			{ 
				...
				unsigned int ncores = 1)
			{
				...
				update_possibly_running_jobs(from,j,start_times,finish_times,core_avail.size(), state_space_data);
			...
			bool try_to_merge(const Schedule_state<Time>& other, bool conservative, bool use_job_finish_times = false)
			{
				...
				merge(other.core_avail, other.job_finish_times, other.certain_jobs, other.possible_jobs, other.earliest_certain_successor_job_disptach, other.tc_data);
			...
			void merge(
				...
				const std::vector<Running_job>& poss_j,
				...
				const TCD<Time>& tc_data_other)
			{
				...
				auto pjt = poss_j.begin();
				auto pjt_end = poss_j.end();
				while (pit != pit_end &&
					pjt != pjt_end) {
					if (pit->idx == pjt->idx) {
						// same job
						new_pj.emplace_back(pit->idx, pit->parallelism | pjt->parallelism, pit->finish_time | pjt->finish_time);
						pit++;
						pjt++;
					}
					else if (pit->idx < pjt->idx){
						new_pj.emplace_back(pit->idx, pit->parallelism, pit->finish_time);
						pit++;
					}else{
						new_pj.emplace_back(pjt->idx, pjt->parallelism, pjt->finish_time);
						pjt++;
					}
				}
				...
				const typename Job<Time>::Job_set& jobs) const
			{
				...
				// Print possibly running jobs
				out << "\\n";
				first = true;
				out << "pr: {";
				for (const auto& pj : possible_jobs) {
					if (!first)
						out << ", ";
					out << "T" << jobs[pj.idx].get_task_id()
						<< "J" << jobs[pj.idx].get_job_id() << ":"
						<< pj.finish_time.min() << "-" << pj.finish_time.max();
					first = false;
				}
				out << "}";
				...
				for(int i=0;i<tc_data.EST_prev.size();i++){ // for task chain i
					...
					for(int j=0;j<tc_data.EIT_Age_int[i].size();j++){ // for task j in chain i
						out << "EIT_Age_int_"<<j<<" = "<<tc_data.EIT_Age_int[i][j]<< ", EIT_Reac_int_"<<j<<" = "<<tc_data.EIT_Reac_int[i][j]<<std::endl;
						if(multiproc) out << "EIT_Age_out_"<<j<<" = "<<tc_data.EIT_Age_out[i][j]<< ", EIT_Reac_out_"<<j<<" = "<<tc_data.EIT_Reac_out[i][j]<<std::endl;
					}
				}
		...
		private:
			...
			TCD<Time> init_task_chain_data_structure(std::vector<Task_chain<Time>> TC_set, bool multiproc){
				...
				for(Task_chain<Time>& tc:TC_set){
					...
					tc_init.EIT_Age_int.emplace_back(tc.get_tasks().size(), initial_value);
					tc_init.EIT_Reac_int.emplace_back(tc.get_tasks().size(), INVALID);
				}
				if(multiproc){
					for(Task_chain<Time>& tc:TC_set){
						tc_init.EIT_Age_out.emplace_back(tc.get_tasks().size(), initial_value);
						tc_init.EIT_Reac_out.emplace_back(tc.get_tasks().size(), INVALID);
					}
				}
				return tc_init;
			}
			...
			void update_task_chain_data(const Schedule_state& from,
				Job_index& idx, const State_space_data<Time>& state_space_data, const Time& EST, const Time& LST, const Time& EFT, const Time& LFT){
					...
					const std::vector<Task_chain<Time>>& TC_set = state_space_data.get_task_chains();
					...
					if(state_space_data.get_num_cpus()==1){ // Single core
						for (const auto& tc_info : state_space_data.get_task_chains_of(tau_j)) {

							bool is_source = tc_info.position_in_chain == 0;
							bool is_sink = tc_info.is_sink;
							auto tc_id = tc_info.chain_id;
							const auto& tc = state_space_data.task_chains[tc_id];
							auto index = tc_info.position_in_chain;
							auto pred_index = index - 1; // wrap-around if index=0 (source task) but pred_index is not used if tau_j is source task

							if (is_source)
								tc_data.EST_prev[tc_id] = EST; // else = from.tc_data.EST_prev[tc_id];

							tc_data.EIT_Age_int[tc_id][index] = is_source ?
								(tc.uses_event_input() ? from.tc_data.EST_prev[tc_id] : EST)
								: from.tc_data.EIT_Age_int[tc_id][pred_index];

							tc_data.EIT_Reac_int[tc_id][index] = from.tc_data.EIT_Reac_int[tc_id][index] == INVALID ?
								(is_source ?
									(tc.uses_event_input() ?
										from.tc_data.EST_prev[tc_id] : EST)
									: from.tc_data.EIT_Reac_int[tc_id][pred_index])
								: (from.tc_data.EIT_Reac_int[tc_id][pred_index] == INVALID ?
									from.tc_data.EIT_Reac_int[tc_id][index] : std::min(from.tc_data.EIT_Reac_int[tc_id][pred_index], from.tc_data.EIT_Reac_int[tc_id][index]));

							if (!is_source) tc_data.EIT_Reac_int[tc_id][pred_index] = INVALID;

							if (is_sink) {
								Time data_age = tc.uses_active_output() ?
									LFT - tc_data.EIT_Age_int[tc_id][index] : LFT - from.tc_data.EIT_Age_int[tc_id][index];
								state_space_data.submit_data_age(tc_id, data_age);
								...
								if (tc_data.EIT_Reac_int[tc_id][index] != INVALID) {
									...
									state_space_data.submit_reaction_time(tc_id, reaction_time);
						...
						for (const auto& tc : TC_set) {
							...
							for (unsigned int tau_l_index = 0; tau_l_index < t.size(); tau_l_index++) { // for all tasks in the chain
								...
								if (!may_have_running_job(tau_l, state_space_data))
								...
								if (from.tc_data.EIT_Reac_int[tc_id][tau_l_index] != INVALID && from.tc_data.EIT_Reac_out[tc_id][tau_l_index] == INVALID && !may_have_running_job(tau_l, state_space_data))
									tc_data.EIT_Reac_out[tc_id][tau_l_index] = from.tc_data.EIT_Reac_int[tc_id][tau_l_index];

								if (tau_j != tau_l)
									tc_data.EIT_Reac_int[tc.get_id()][tau_l_index] = may_have_running_job(tau_l, state_space_data) ?
						...
						for (const auto& tc_info : state_space_data.get_task_chains_of(tau_j)) {
							...
							tc_data.EIT_Age_int[tc_id][index] = is_source ?
								(tc.uses_event_input() ? from.tc_data.EST_prev[tc_id] : EST)
								: (may_have_running_job(t[pred_index], state_space_data) ? from.tc_data.EIT_Age_int[tc_id][pred_index] : from.tc_data.EIT_Age_int[tc_id][pred_index]);

							tc_data.EIT_Reac_int[tc_id][index] = from.tc_data.EIT_Reac_int[tc_id][index] == INVALID ?
								(is_source ?
									(tc.uses_event_input() ?
										from.tc_data.EST_prev[tc_id] : EST)
									: (may_have_running_job(t[pred_index], state_space_data) || from.tc_data.EIT_Reac_int[tc_id][pred_index] == INVALID ?
