...
def max_age_local(chain, task_set, schedule_wcet, schedule_bcet):
    ...
    for idx in itertools.count(start=1):
        ...
        # Break loop
        if (chain[0].phase + abstr[0] * chain[0].period) >= (max_phase + 2*hyper):
            break
...
def mrt_let(chain, task_set):
    ...
    for idx in itertools.count():
        # check valid
        if not (ana.rel(chain[0], idx + 1) >= max_first_read_event):
            continue

        # Compute idx-th fw chain
        fwidx = []

        fwidx.append(ana.rel(chain[0], idx))  # external activity
        fwidx.append(idx+1)  # first job

        for curr_task, nxt_task in zip(chain[:-1], chain[1:]):
            fwidx.append(  # add next release
                ana.find_next_rel(nxt_task, ana.dl(curr_task, fwidx[-1]))
            )

        fwidx.append(ana.dl(chain[-1], fwidx[-1]))  # actuation
        ...
        # break condition
        if ana.rel(chain[0], idx) >= (max_phase + 2*hyper):
            break
...
def mda_let(chain, task_set):
    ...
    for idx in itertools.count(start=1):
        ...
        bwidx.append(ana.dl(chain[-1], idx))  # actuation
        bwidx.append(idx-1)  # last job

        for curr_task, prev_task in zip(chain[::-1][:-1], chain[::-1][1:]):
        ...
        bwidx.append(ana.rel(chain[0], bwidx[-1]))  # actuation
        ...
        # check valid
        if not (ana.rel(chain[0], bwidx[1]) >= max_first_read_event):
            continue

        # break condition
        if ana.rel(chain[0], bwidx[1]) >= (max_phase + 2*hyper):
            break
    ...
    # maximal reduced length
    max_length_red = max(
        ana.dl(chain[-1], bwidx[-2]) - bwidx[0] for bwidx in bw)
